...
export function POActions({
  ...
  const handleView = () => {
    ...
    if (programId) {
      router.push(`/dashboard/admin/programs/${programId}/pos/${po.po_id}`);
    } else {
  ...
  const handleCopy = async () => {
    try {
      ...
      toast.success(
        "Thông tin mục tiêu đào tạo đã được sao chép vào clipboard"
      );
    } catch (error) {
  ...
  const handleExport = () => {
    ...
    toast.success("Thông tin mục tiêu đào tạo đã được xuất thành file JSON");
  };
  ...
  const handleDelete = async () => {
    try {
      ...
      if (response.success) {
        toast.success("Mục tiêu đào tạo đã được xóa thành công");
        onUpdate?.();
      }
    } catch (error) {
      toast.error("Không thể xóa mục tiêu đào tạo");
    } finally {
      setIsDeleting(false);
